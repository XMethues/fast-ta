---
date: 2026-07-05T09:51:36+0800
author: unknown
commit: 896d1d7
branch: main
repository: fast-ta
topic: "TA-Lib Rust core foundation + first tranche"
tags: [plan, ta-core, talib, indicators, foundation, first-tranche]
status: ready
parent: ".rpiv/artifacts/designs/2026-07-04_17-28-24_rust-talib-core-foundation-first-tranche.md"
phase_count: 7
phases:
  - { n: 1, title: Core contracts }
  - { n: 2, title: SMA rewrite }
  - { n: 3, title: Price Transform group }
  - { n: 4, title: Math Transform group }
  - { n: 5, title: Math Operators group }
  - { n: 6, title: Full inventory ledger }
  - { n: 7, title: First-tranche benchmarks }
last_updated: 2026-07-05T09:51:36+0800
last_updated_by: unknown
---

# TA-Lib Rust Core Foundation + First Tranche Implementation Plan

## Overview

Implement the TA-Lib Rust core foundation and first tranche from the parent design artifact. The work establishes shared compact-output helpers, updates the `Indicator` contract, rewrites `SMA`, adds Price Transform, Math Transform, Math Operators, records the official 161-function source inventory, and benchmarks the new public APIs. Phase boundaries are inherited 1:1 from the design's `## Slices` section.

## Desired End State

The core exposes TA-Lib-style zero-copy compact functions returning `OutputRange`, padded `*_vec` convenience wrappers, and uppercase struct surfaces. `SMA`, Price Transform, Math Transform, Math Operators, and inventory APIs compile and pass their integration tests; first-tranche Criterion benchmarks compile against public `ta_core` APIs.
...
- Python (`ta-py`) and WASM (`ta-wasm`) bindings in this plan; they consume stable core APIs later.
...
### Overview
Add shared output ranges, padding/validation helpers, zero-copy trait contract, and root exports required by all later indicator phases.

### Changes Required:

#### 1. Shared core helpers
**File**: `crates/ta-core/src/common.rs`
**Changes**: Create shared `OutputRange`, `PadValue`, output sizing, validation, compact-to-padded, and compact-buffer helpers.
...
#### 2. Indicator trait contract
**File**: `crates/ta-core/src/traits.rs`
**Changes**: Rewrite trait docs and add default zero-copy `compute` plus padded `compute_to_vec` behavior.
...
#### 3. Core facade exports
**File**: `crates/ta-core/src/lib.rs`
**Changes**: Add the `common` module and shared helper re-exports needed by the updated trait contract.
...
- [x] Core crate compiles after helper and trait additions: `cargo test -p ta-core`
- [x] f32 precision still compiles with the new helpers: `cargo test -p ta-core --features f32`
- [x] Workspace module exports still check after `lib.rs` changes: `cargo check --workspace`
- [x] Formatting passes: `cargo fmt --all -- --check`
- [x] New zero-copy trait method is present: `grep -n "fn compute(&self" crates/ta-core/src/traits.rs`
...
### Overview
Rewrite `SMA` onto the dual API, add fallible construction, initialize streaming state safely, and cover the new behavior with integration tests.

### Changes Required:

#### 1. SMA module re-export
**File**: `crates/ta-core/src/overlap/mod.rs`
**Changes**: Re-export rewritten SMA free function, padded wrapper, and struct surface.
...
#### 2. SMA implementation
**File**: `crates/ta-core/src/overlap/sma.rs`
**Changes**: Rewrite SMA as compact free function, padded wrapper, fallible struct, checked streaming update, and `Indicator`/`Resettable` implementation.
...
#### 3. SMA tests
**File**: `crates/ta-core/tests/overlap_sma.rs`
**Changes**: Add integration coverage for compact outputs, padded wrapper, struct compute, validation failures, streaming, and reset.
...
#### Automated Verification:
- [x] SMA rewrite tests pass: `cargo test -p ta-core --test overlap_sma`
- [x] Existing core tests still pass: `cargo test -p ta-core`
- [x] f32 precision still compiles: `cargo test -p ta-core --features f32`
- [x] Workspace public exports still check after `overlap/mod.rs` changes: `cargo check --workspace`
- [x] Formatting passes: `cargo fmt --all -- --check`
- [x] No SMA period constructor panic remains: `grep -R "assert!(.*period" crates/ta-core/src/overlap/sma.rs` returns no matches
...
## Phase 3: Price Transform group

### Overview
Add all five official Price Transform functions with compact APIs, padded wrappers, uppercase struct surfaces, integration tests, and root module wiring.

### Changes Required:

#### 1. Price Transform module wiring
**File**: `crates/ta-core/src/price_transform/mod.rs`
**Changes**: Create the family module and public re-exports for all Price Transform functions.
...
#### 2. AVGDEV implementation
**File**: `crates/ta-core/src/price_transform/avgdev.rs`
**Changes**: Implement rolling Average Deviation compact API, padded wrapper, uppercase struct, and trait implementation.
...
#### 3. AVGPRICE implementation
**File**: `crates/ta-core/src/price_transform/avgprice.rs`
**Changes**: Implement Average Price with separate OHLC slices, padded wrapper, and uppercase struct surface.
...
#### 4. MEDPRICE implementation
**File**: `crates/ta-core/src/price_transform/medprice.rs`
**Changes**: Implement Median Price with separate high/low slices, padded wrapper, and uppercase struct surface.
...
#### 5. TYPPRICE implementation
**File**: `crates/ta-core/src/price_transform/typprice.rs`
**Changes**: Implement Typical Price with separate HLC slices, padded wrapper, and uppercase struct surface.
...
#### 6. WCLPRICE implementation
**File**: `crates/ta-core/src/price_transform/wclprice.rs`
**Changes**: Implement Weighted Close Price with separate HLC slices, padded wrapper, and uppercase struct surface.
...
#### 7. Price Transform tests
**File**: `crates/ta-core/tests/price_transform.rs`
**Changes**: Add expected-value, wrapper-shape, struct-surface, and validation tests.
...
#### 8. Core facade export
**File**: `crates/ta-core/src/lib.rs`
**Changes**: Ensure `price_transform` is publicly wired in the root facade.
...
#### Automated Verification:
- [x] Price Transform tests pass: `cargo test -p ta-core --test price_transform`
- [x] Existing SMA tests still pass: `cargo test -p ta-core --test overlap_sma`
- [x] Core crate tests pass after `lib.rs` module addition: `cargo test -p ta-core`
- [x] f32 precision still compiles: `cargo test -p ta-core --features f32`
- [x] Workspace exports still check after adding `price_transform`: `cargo check --workspace`
- [x] Formatting passes: `cargo fmt --all -- --check`
- [x] Public Price Transform module is wired: `grep -n "pub mod price_transform" crates/ta-core/src/lib.rs`
- [x] Stateless constructors are fallible: `grep -R "pub const fn new() -> Self" crates/ta-core/src/price_transform` returns no matches
...
## Phase 4: Math Transform group

### Overview
Add all 15 official unary Math Transform functions with compact APIs, padded wrappers, uppercase struct surfaces, tests, and root module wiring.

### Changes Required:

#### 1. Math Transform module
**File**: `crates/ta-core/src/math_transform/mod.rs`
**Changes**: Create macro-backed unary transforms, wrapper functions, struct surfaces, trait implementations, and public re-exports.
...
**File**: `crates/ta-core/tests/math_transform.rs`
**Changes**: Add expected-value, export coverage, padded wrapper, struct-surface, and validation tests.
...
#### 3. Core facade export
**File**: `crates/ta-core/src/lib.rs`
**Changes**: Ensure `math_transform` is publicly wired in the root facade.
...
#### Automated Verification:
- [x] Math Transform tests pass: `cargo test -p ta-core --test math_transform`
- [x] Price Transform tests still pass: `cargo test -p ta-core --test price_transform`
- [x] SMA tests still pass: `cargo test -p ta-core --test overlap_sma`
- [x] Core crate tests pass after `lib.rs` module addition: `cargo test -p ta-core`
- [x] f32 precision still compiles: `cargo test -p ta-core --features f32`
- [x] Workspace exports still check after adding `math_transform`: `cargo check --workspace`
- [x] Formatting passes: `cargo fmt --all -- --check`
- [x] Public Math Transform module is wired: `grep -n "pub mod math_transform" crates/ta-core/src/lib.rs`
- [x] Math Transform follows private-module re-export direction: `grep -n "pub use functions" crates/ta-core/src/math_transform/mod.rs`
...
## Phase 5: Math Operators group

### Overview
Add all 11 official Math Operators with compact APIs, padded wrappers, uppercase struct surfaces, tests, and root module wiring.

### Changes Required:

#### 1. Math Operators module wiring
**File**: `crates/ta-core/src/math_operators/mod.rs`
**Changes**: Create the family module and public re-exports for arithmetic, rolling, and extrema operators.
...
**File**: `crates/ta-core/src/math_operators/arithmetic.rs`
**Changes**: Implement `ADD`, `SUB`, `MULT`, and `DIV` with separate slices, padded wrappers, and struct surfaces.
...
#### 3. Rolling operators
**File**: `crates/ta-core/src/math_operators/rolling.rs`
**Changes**: Implement `SUM`, `MIN`, and `MAX` rolling-window APIs, padded wrappers, stateful structs, and resets.
...
#### 4. Extrema operators
**File**: `crates/ta-core/src/math_operators/extrema.rs`
**Changes**: Implement `MININDEX`, `MAXINDEX`, `MINMAX`, and `MINMAXINDEX` with integer/parallel outputs, wrappers, structs, and resets.
...
#### 5. Math Operators tests
**File**: `crates/ta-core/tests/math_operators.rs`
**Changes**: Add expected-value, rolling, extrema-index, vector-wrapper, struct-surface, and validation tests.
...
#### 6. Core facade export
**File**: `crates/ta-core/src/lib.rs`
**Changes**: Ensure `math_operators` is publicly wired in the root facade.
...
#### Automated Verification:
- [x] Math Operators tests pass: `cargo test -p ta-core --test math_operators`
- [x] Math Transform tests still pass: `cargo test -p ta-core --test math_transform`
- [x] Price Transform tests still pass: `cargo test -p ta-core --test price_transform`
- [x] SMA tests still pass: `cargo test -p ta-core --test overlap_sma`
- [x] Core crate tests pass after `lib.rs` module addition: `cargo test -p ta-core`
- [x] f32 precision still compiles for arithmetic/operators: `cargo test -p ta-core --features f32`
- [x] Workspace exports still check after adding `math_operators`: `cargo check --workspace`
- [x] Formatting passes: `cargo fmt --all -- --check`
- [x] Public Math Operators module is wired: `grep -n "pub mod math_operators" crates/ta-core/src/lib.rs`
- [x] Math Operators follow private-module re-export direction: `grep -n "pub use arithmetic" crates/ta-core/src/math_operators/mod.rs`
- [x] First-tranche implementations do not call private SIMD/architecture backends: `grep -R "crate::simd::arch\|core::arch\|std::arch" crates/ta-core/src/overlap crates/ta-core/src/price_transform crates/ta-core/src/math_transform crates/ta-core/src/math_operators` returns no matches
...
## Phase 6: Full inventory ledger

### Overview
Add a source-level official 161-function TA-Lib ledger with group counts, implemented/planned status, tests, and root module wiring.

### Changes Required:

#### 1. Inventory ledger
**File**: `crates/ta-core/src/inventory.rs`
**Changes**: Create `FunctionGroup`, `ImplementationStatus`, `FunctionInfo`, explicit group counts, and `TALIB_FUNCTIONS` records.
...
**Changes**: Add count, group, uniqueness, implemented-status, planned-status, and non-TA-Lib-extra tests.
...
#### 3. Core facade export
**File**: `crates/ta-core/src/lib.rs`
**Changes**: Ensure `inventory` is publicly wired in the root facade.
...
#### Automated Verification:
- [x] Inventory tests pass: `cargo test -p ta-core --test inventory`
- [x] Math Operators tests still pass: `cargo test -p ta-core --test math_operators`
- [x] Math Transform tests still pass: `cargo test -p ta-core --test math_transform`
- [x] Price Transform tests still pass: `cargo test -p ta-core --test price_transform`
- [x] SMA tests still pass: `cargo test -p ta-core --test overlap_sma`
- [x] Core crate tests pass after `lib.rs` module addition: `cargo test -p ta-core`
- [x] Workspace exports still check after adding `inventory`: `cargo check --workspace`
- [x] Formatting passes: `cargo fmt --all -- --check`
- [x] Public inventory module is wired: `grep -n "pub mod inventory" crates/ta-core/src/lib.rs`
- [x] Source ledger count remains explicit: `grep -n "FUNCTION_COUNT: usize = 161" crates/ta-core/src/inventory.rs`
...
## Phase 7: First-tranche benchmarks

### Overview
Register and implement a Criterion benchmark target covering public first-tranche APIs for SMA, Price Transform, Math Transform, and Math Operators.

### Changes Required:

#### 1. Benchmark target registration
**File**: `crates/ta-benchmarks/Cargo.toml`
**Changes**: Register `first_tranche` as a Criterion benchmark target while preserving `basic`.
...
#### 2. First-tranche benchmarks
**File**: `crates/ta-benchmarks/benches/first_tranche.rs`
**Changes**: Add Criterion fixtures and benchmark groups for compact kernels plus selected padded wrapper allocation benchmarks.
...
#### Automated Verification:
- [x] First-tranche benchmark target compiles: `cargo bench -p ta-benchmarks --bench first_tranche --no-run`
- [x] Existing basic benchmark target still compiles: `cargo bench -p ta-benchmarks --bench basic --no-run`
- [x] Core crate tests still pass after benchmark additions: `cargo test -p ta-core`
- [x] Workspace exports still check with benchmark target registered: `cargo check --workspace`
- [x] Formatting passes: `cargo fmt --all -- --check`
- [x] First-tranche benchmark target is registered: `grep -n "name = \"first_tranche\"" crates/ta-benchmarks/Cargo.toml`
...
- Run `cargo test -p ta-core --features f32` after Phase 1 and after each arithmetic/transform phase.
- Run `cargo check --workspace` after module export changes; note `ta-py` may require a Python environment.
- Run `cargo bench -p ta-benchmarks --bench first_tranche --no-run` after Phase 7.
...
- Verify full inventory remains 161 records once Phase 6 lands.
...
1. Confirm compact output helpers document and preserve TA-Lib-style `outBegIdx/outNBElement` mapping.
2. Confirm padded wrappers preserve full input length with warm-up sentinels.
3. Confirm strict finite-input validation returns `TalibError` for invalid inputs.
4. Confirm all first-tranche public APIs use uppercase TA-Lib names and separate slices for multi-price or paired-series inputs.
5. Confirm inventory status values match implemented APIs and official TA-Lib group counts.
6. Confirm benchmark code uses only public `ta_core` APIs and allocates outside `b.iter()` except wrapper allocation measurements.
...
| ------ | -------- | ------------ | -------- | --------- | ------- | -------------- | ---------- |
| code | Phase 1 §3 (lib.rs) | &lt;n/a&gt; | blocker | actionability | Phase 1 declares `pub mod inventory`, `pub mod math_operators`, `pub mod math_transform`, and `pub mod price_transform` before those modules are created, so Phase 1 compile stops with missing-module errors | In Phase 1 only add `common`; add each later module declaration in the phase that creates that module | applied (plan-local; design follow-up: `.rpiv/artifacts/designs/2026-07-04_17-28-24_rust-talib-core-foundation-first-tranche.md`): Phase 1 now wires `common` only; later facade fences add modules when their files exist |
| code | Phase 3 §8 (lib.rs) | &lt;n/a&gt; | blocker | actionability | Phase 3's facade still declares future `inventory`, `math_operators`, and `math_transform` modules before their files exist | In Phase 3 only add `price_transform` beyond the modules already scaffolded | applied (plan-local; design follow-up: `.rpiv/artifacts/designs/2026-07-04_17-28-24_rust-talib-core-foundation-first-tranche.md`): Phase 3 facade now adds only `price_transform` beyond prior modules |
| code | Phase 4 §3 (lib.rs) | &lt;n/a&gt; | blocker | actionability | Phase 4's facade declares future `inventory` and `math_operators` modules before their files exist | In Phase 4 only add `math_transform` beyond the modules already scaffolded | applied (plan-local; design follow-up: `.rpiv/artifacts/designs/2026-07-04_17-28-24_rust-talib-core-foundation-first-tranche.md`): Phase 4 facade now adds only `math_transform` beyond prior modules |
| code | Phase 5 §6 (lib.rs) | &lt;n/a&gt; | blocker | actionability | Phase 5's facade declares `pub mod inventory` before `crates/ta-core/src/inventory.rs` is created in Phase 6 | Move the `inventory` module declaration and re-export wiring to Phase 6 | applied (plan-local; design follow-up: `.rpiv/artifacts/designs/2026-07-04_17-28-24_rust-talib-core-foundation-first-tranche.md`): Phase 5 no longer declares `inventory`; Phase 6 keeps final inventory wiring |
| coverage | ## Testing Strategy §8 | &lt;n/a&gt; | blocker | verification-coverage | Entry "Preserve cross-platform SIMD dispatch boundaries; first-tranche functions may use scalar loops or public SIMD helpers but must not call private arch backends." — criteria NOT FOUND and code NOT FOUND; no Success Criteria bullet greps for private arch calls and no code-level guard/assertion enforces the boundary | Add an automated bullet under Phase 5's `#### Automated Verification:`: `grep -R "crate::simd::arch&#124;core::arch&#124;std::arch" crates/ta-core/src/overlap crates/ta-core/src/price_transform crates/ta-core/src/math_transform crates/ta-core/src/math_operators` returns no matches | applied: added Phase 5 automated SIMD-boundary grep success criterion |
| code | Phase 1 §2 (traits.rs) | &lt;n/a&gt; | concern | code-quality | The default `Indicator::compute` uses `output_count` instead of `validate_input_len`, so a non-empty insufficient input can return `Ok(OutputRange::new(lookback, 0))` if `compute_to_vec` pads instead of errors | Replace the `output_count` call with `validate_input_len(inputs.len(), lookback)?` and use the returned count | applied (plan-local; design follow-up: `.rpiv/artifacts/designs/2026-07-04_17-28-24_rust-talib-core-foundation-first-tranche.md`): Phase 1 `traits.rs` code fence now imports and calls `validate_input_len` |
| code | Phase 3 §2 (avgdev.rs) | crates/ta-core/src/traits.rs:249 | concern | codebase-fit | `AVGDEV` owns streaming state (`buffer`, `index`, `count`) but does not implement the crate's `Resettable` trait | Add `impl Resettable for AVGDEV` that clears the buffer, index, and count | applied (plan-local; design follow-up: `.rpiv/artifacts/designs/2026-07-04_17-28-24_rust-talib-core-foundation-first-tranche.md`): Phase 3 `avgdev.rs` code fence now imports `Resettable` and clears buffer/index/count in `impl Resettable` |
...
- Design: `.rpiv/artifacts/designs/2026-07-04_17-28-24_rust-talib-core-foundation-first-tranche.md`
- Research: `.rpiv/artifacts/research/2026-07-04_15-40-32_rust-talib-core-inventory.md`
- Official TA-Lib function index: `https://ta-lib.org/functions/`
- TA-Lib v0.7.1 function metadata XML: `https://github.com/TA-Lib/ta-lib/blob/v0.7.1/ta_func_api.xml`
- TA-Lib v0.7.1 generated C function header: `https://github.com/TA-Lib/ta-lib/blob/v0.7.1/include/ta_func.h`
- ta-lib-python generated wrappers: `https://github.com/TA-Lib/ta-lib-python/blob/master/tools/generate_func.py`
